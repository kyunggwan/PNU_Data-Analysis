...
# 실습: 탐색기를 통해서 파일 선택하기 (여기는 탭으로 칸을 구분)
student1 <- read.table(file.choose(), header = TRUE, fileEncoding = "euc-kr")


# 실습: 구분자가 있는 경우 (여기는 ;로 칸을 구분)
student2 <- read.table(file = "student2.txt", sep = ";", header = TRUE, fileEncoding = "euc-kr")

# student2 <- read.table(file = "student2.txt", sep = "\t", header = TRUE, fileEncoding = "euc-kr")


# 실습: 결측치를 처리하여 파일 불러오기 (값이 빈 것에 - 넣음, NA로 표시됨)
student3 <- read.table(file = "student3.txt", header = TRUE, na.strings = "-", fileEncoding = "euc-kr")
student3

# 실습: CSV 파일 형식 불러오기 (값이 빈칸에는 - 가 있다. 구분을, 로 한다.)
student4 <- read.csv(file = "student4.txt", sep = ",", na.strings = "-", fileEncoding = "euc-kr")
student4
