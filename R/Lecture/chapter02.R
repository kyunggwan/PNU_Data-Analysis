...
getwd()
txtemp <- read.table('emp.txt', header = 1, sep = "", fileEncoding = "euc-kr")
txtemp


# 실습: csv 파일을 이용한 데이터프레임 객체 생성하기 
csvtemp <- read.csv('emp.csv', header = T, fileEncoding = "euc-kr")
csvtemp
help(read.csv)
name <- c("사번", "이름", "급여")
read.csv('emp2.csv', header = F, col.names = name, fileEncoding = "euc-kr")
